# Event Registration System

A Java desktop application for managing events and participant registrations, built using **Swing**, **AWT**, and standard Java UI components.

## 📌 Project Overview

This application allows users to:

- Create, update, and delete **events**  
- Register **participants** for events  
- View and search events and participants  
- Store data persistently using `.ser` files (serialization)

This project demonstrates practical use of Java OOP principles, GUI design, and file-based persistence for desktop applications. 

## 🚀 Features

✔ Add new events  
✔ Add participants  
✔ Register participants for events  
✔ Search and sort lists  
✔ Simple and intuitive UI using Java Swing  
✔ Persistent data storage with `.ser` files (no database needed)

## 📁 Technologies Used

- **Java (JDK 8+)**
- **Java Swing & AWT** (UI)
- **Serialization** for file-based data storage  
- **MVC-style application structure**

---

## 🛠️ How to Run

1. Clone the repository:

   ```bash
   git clone https://github.com/nafisa3003/Event-Registration-System.git
